        ...
        <div id="experience" className="Experience" class="w-full mt-36">
          ...
          <div class="relative mx-80 z-2 -mt-96">
            {experience.map((job) => {
              ...
              return (<div class="flex mt-10">
                <h2 class="text-9xl text-black opacity-60 hover:opacity-100" onClick={() => this.handleClick(job.key)}>{job.text}</h2>
                <div class="flex ml-5 -mt-2">
                  <h3 class="text-ironmanred mr-2 text-xl">{job.start}</h3>
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                  </svg>
                  <h3 class="ml-2 text-xl">{job.end}</h3>
                </div>
              </div>)
            })
            }
